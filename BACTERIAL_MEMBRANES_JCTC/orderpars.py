...
################################
            if (xtc_file=='none'): # Single pdb file
                C=(trj.select_atoms("resname "+resname+" adn (name "+Catoms[c][0]+" or name "+Catoms_per[c][0]+")").positions)
                for i in range (1,len(Catoms[c]),1):
                    if (AA_UA=='AA'): H.append(trj.select_atoms("resname "+resname+" and (name "+Catoms[c][i]+" or name "+Catoms_per[c][i]+")").positions)##### AA
                    else: X.append(trj.select_atoms("resname "+resname+" and (name "+Catoms[c][i]+" or name "+Catoms_per[c][i]+")").positions)
                for i in range (1,len(C2atoms[c]),1):
                    X2.append(trj.select_atoms("resname "+resname+" and (name "+C2atoms[c][i]+" or name "+C2atoms_per[c][i]+")").positions)
                ...
                #dt = trj.trajectory[1].time - trj.trajectory[0].time
                #begin = int(begin/(dt*1e-3))
                #end = int(end/(dt*1e-3))
                for frame in trj.trajectory[begin:end:skip]: # Iterate over frames 
                    fr=fr+1; H=[]; X=[]; X2=[]#; op_f=[];
                    print ".",
                    C=(trj.select_atoms("resname "+resname+" and (name "+Catoms[c][0]+" or name "+Catoms_per[c][0]+")").positions)
                    for i in range (1,len(Catoms[c]),1):
                        if (AA_UA=='AA'): H.append(trj.select_atoms("resname "+resname+" and (name "+Catoms[c][i]+" or name "+Catoms_per[c][i]+")").positions)##### AA
                        else: X.append(trj.select_atoms("resname "+resname+" and (name "+Catoms[c][i]+" or name "+Catoms_per[c][i]+")").positions)
                    for i in range (1,len(C2atoms[c]),1):
